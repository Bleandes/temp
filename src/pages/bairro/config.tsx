...
export function useBairroTableConfig() {
  ...
  async function deleteOnClick(id: GridRowId) {
    await api.excluirBairro(id.toString(), () => {
      triggerReload();
  ...
  function generateConfig() {
    return [
      ...
      },
      {
        ...
        flex: 0.2,
        type: 'actions',
        getActions: (params: GridRowParams<IBairro>) => [
          <>
            {hasPermissaoById('/bairro/details') && (
              <GridActionsCellItem
                label="Visualizar"
                icon={<VisibilityIcon />}
                onClick={() => navigate(`/bairro/details`, {state: {id: params.id}})}
                title={`${translate('global.show')}`}
              />
            )}
          </>,
          // <>
          //   {hasPermissaoById('/bairro/edit') && (
          //     <GridActionsCellItem
          //       label="Editar"
          //       icon={<ModeEditIcon />}
          //       onClick={() => navigate(`/bairro/edit`, {state: {id: params.id}})}
          //       title={`${translate('global.edit')}`}
          //     />
          //   )}
          // </>,
          // <>
          //   {hasPermissaoById('/bairro/delete') && (
          //     <GridActionsCellItem
          //       label="Excluir"
          //       icon={<DeleteIcon />}
          //       onClick={() =>
