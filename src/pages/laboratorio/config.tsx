...
export function useLaboratorioTableConfig() {
  ...
  async function deleteOnClick(id: GridRowId) {
    await api.excluirLaboratorio(id.toString(), () => {
      triggerReload();
  ...
  function generateConfig() {
    return [
      ...
      },
      {
        ...
        flex: 0.2,
        type: 'actions',
        getActions: (params: GridRowParams<ILaboratorio>) => [
          <>
            {hasPermissaoById('/laboratorio/details') && (
              <GridActionsCellItem
                label="Visualizar"
                icon={<VisibilityIcon />}
                onClick={() => navigate(`/laboratorio/details`, {state: {id: params.id}})}
                title={`${translate('global.show')}`}
              />
            )}
          </>,
        //   <>
        //     {hasPermissaoById('/laboratorio/edit') && (
        //       <GridActionsCellItem
        //         label="Editar"
        //         icon={<ModeEditIcon />}
        //         onClick={() => navigate(`/laboratorio/edit`, {state: {id: params.id}})}
        //         title={`${translate('global.edit')}`}
        //       />
        //     )}
        //   </>,
        //   <>
        //     {hasPermissaoById('/laboratorio/delete') && (
        //       <GridActionsCellItem
        //         label="Excluir"
        //         icon={<DeleteIcon />}
        //         onClick={() =>
