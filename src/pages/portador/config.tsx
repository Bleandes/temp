...
export function usePortadorTableConfig() {
  ...
  async function deleteOnClick(id: GridRowId) {
    await api.excluirPortador(id.toString(), () => {
      triggerReload();
    ...
    return [
      ...
      },
      {
        ...
        getActions: (params: GridRowParams) => [
          <>
            {hasPermissaoById('/portador/details') && (
              <GridActionsCellItem
                label="Visualizar"
                icon={<VisibilityIcon />}
                onClick={() => navigate(`/portador/details`, {state: {id: params.id}})}
                title={`${translate('global.show')}`}
              />
            )}
          </>,
          // <>
          //   {hasPermissaoById('/portador/edit') && (
          //     <GridActionsCellItem
          //       label="Editar"
          //       icon={<ModeEditIcon />}
          //       onClick={() => navigate(`/portador/edit`, {state: {id: params.id}})}
          //       title={`${translate('global.edit')}`}
          //     />
          //   )}
          // </>,
          // <>
          //   {hasPermissaoById('/portador/delete') && (
          //     <GridActionsCellItem
          //       label="Excluir"
          //       icon={<DeleteIcon />}
          //       onClick={() =>
